...
class TestRuntimes:
    ...
    def test_create_bad_json_missing_runtime_type(self, testapp, users):
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=users["fake_superadmin"]):
    ...
    def test_create_bad_json_missing_description(self, testapp, users):
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=users["fake_superadmin"]):
    ...
    def test_create_bad_json_missing_runtime_familly(self, testapp, users):
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=users["fake_superadmin"]):
    ...
    #################################
    # Response 200:
    def test_get_runtime(self, testapp, users):
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=users["fake_user"]):
    ...
    # Response 401:
    def test_get_runtime_unauthenticated(self, testapp, users):
        runtime_id = get_runtime_id(testapp, users)
        # Get this runtime by id
        testapp.get(api_version + "/runtimes/" + runtime_id, status=401)

    # Response 404:
    def test_get_bad_runtime(self, testapp, users):
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=users["fake_user"]):
    ...
    #################################
    # Response 200:
    def test_update_runtime(self, testapp, users):
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=users["fake_superadmin"]):
    ...
    # Response 201:
    def test_update_unexisting_runtime(self, testapp, users):
        with patch.object(oidc, 'validate_token', return_value=True), \
            patch("utils.check_user_role", return_value=users["fake_superadmin"]):
    ...
    # Response 400:
    def test_update_bad_runtime(self, testapp, users):
        with patch.object(oidc, 'validate_token', return_value=True), \
            patch("utils.check_user_role", return_value=users["fake_superadmin"]):
    ...
    # Response 403:
    def test_update_insufficient_rights(self, testapp, users):
        runtime_id = get_runtime_id(testapp, users)
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", side_effect=Forbidden):
    ...
    def test_delete_runtime(self, testapp, users):
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=users["fake_superadmin"]):
