...
from models import RoleEnum, webapp_schema
import json
import pytest
...
class TestCapsuleOwners:
    ...
    def test_delete_unauthorized(self, testapp, db):
        capsule_id = str(db.capsule1.id)
        testapp.delete(api_version + "/capsules/" + capsule_id + "/owners/" + db.user2.name, status=401)
    ...
    def test_delete_not_found_capsule_id(self, testapp, db):
        with patch.object(oidc, "validate_token", return_value=True), \
            ...
            testapp.delete(api_version + "/capsules/" + unexisting_id + "/owners/whatever", status=404)
    ...
    def test_delete_conflict(self,testapp, db):
        ...
        with patch.object(oidc, "validate_token", return_value=True), \
            ...
            testapp.delete(api_version + "/capsules/" + capsule_id + "/owners/" + db.user1.name, status=409)
    ...
    def test_delete(self, testapp, db):
        ...
        with patch.object(oidc, "validate_token", return_value=True), \
            ...
            # Delete owner
            testapp.delete(api_version + "/capsules/" + capsule_id + "/owners/" + db.user2.name, status=204)
