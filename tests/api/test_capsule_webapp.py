...
class TestCapsuleWebapp:
    ...
    @classmethod
    def build_webapp(cls, testapp, users):
        runtime_id = get_runtime_id(testapp, users)
    ...
    ################################################
    # Response 400:
    def test_create_bad_capsule_id(self, testapp, users):
    ...
    def test_create_missing_runtime_id(self, testapp, users):
        with patch.object(oidc, 'validate_token', return_value=True), \
            patch("utils.check_user_role", return_value=foobar):
    ...
    def test_create_missing_fqdns(self, testapp, users):
        with patch.object(oidc, 'validate_token', return_value=True), \
            patch("utils.check_user_role", return_value=foobar):
            capsule_id = get_capsule_id(testapp, users)

            # Build webapp with correct runtime_id but no fqdns
            new_webapp = self.build_webapp(testapp, users)
    ...
    # Response 403:
    def test_create_bad_owner(self, testapp, users):
        with patch.object(oidc, "validate_token", return_value=True), \
    ...
    # Response 409:
    def test_create_conflict(self, testapp, users):
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=foobar):
            capsule_id = get_capsule_id(testapp, users)
            new_webapp = self.build_webapp(testapp, users)
    ...
    ################################################
    # Response 200:
    def test_update(self, testapp, users):
        capsule_id = get_capsule_id(testapp, users)
        runtime_id = get_runtime_id(testapp, users)
    ...
    # Response 201:
    @pytest.mark.filterwarnings("ignore:.*Content-Type header found in a 204 response.*:Warning")
    def test_update_unexisting_webapp(self, testapp, users):
        capsule_id = get_capsule_id(testapp, users)
    ...
    # Response 401:
    def test_update_unauthenticated(self, testapp, users):
        capsule_id = get_capsule_id(testapp, users)
        new_webapp = self.build_webapp(testapp, users)
        testapp.put_json(api_version + "/capsules/" + capsule_id + '/webapp', new_webapp, status=401)

    # Response 400:
    def test_update_bad_request(self, testapp, users):
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=foobar):
    ...
    # Response 403:
    def test_update_bad_owner(self, testapp, users):
        capsule_id = get_capsule_id(testapp, users)
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=users["fake_user"]):
    ...
    # Response 403:
    def test_delete_bad_owner(self, testapp, users):
        capsule_id = get_capsule_id(testapp, users)
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=users["fake_user"]):
    ...
    @pytest.mark.filterwarnings("ignore:.*Content-Type header found in a 204 response.*:Warning")
    def test_delete_no_webapp(self, testapp, users):
        capsule_id = get_capsule_id(testapp, users)
        with patch.object(oidc, "validate_token", return_value=True), \
            patch("utils.check_user_role", return_value=foobar), \
